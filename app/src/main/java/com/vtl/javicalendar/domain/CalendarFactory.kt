...
import com.vtl.javicalendar.presentation.model.*
...
import com.vtl.javicalendar.utils.LunarCalendarUtils.lunarMonthName
...
object CalendarFactory {
  ...
  fun createMonthInfo(
    ...
    return MonthInfo(weekDayNames = weekDayNames, weeks = weeks)
  }
  ...
  private fun createDateInfo(
    ...
    // Get lunar date
    val lunarDate =
        LunarCalendarUtils.convertSolarToLunar(date.dayOfMonth, date.monthValue, date.year)
    ...
    // Lunar year display
    val lunarYearDisplay =
        if (lunarDate.year != date.year) {
    ...
    return DateInfo(
        ...
        // Gregorian date
        year = DisplayField(value = date.year.toString(), color = Color.Unspecified),
        month = DisplayField(value = monthName, color = Color.Unspecified),
        day =
            DisplayField(
        ...
        japaneseDate =
            JapaneseDateDisplay(
                year = DisplayField(value = "($japaneseYear)", color = japaneseYearColor),
                holiday = japaneseHoliday?.let { DisplayField(value = it, color = HolidayRed) },
            ),

        // Lunar date
        lunarDate =
            LunarDateDisplay(
                year = DisplayField(value = "($lunarYearDisplay)", color = Color.Unspecified),
                month = DisplayField(value = "($lunarMonthNameStr)", color = Color.Unspecified),
                day =
                    DisplayField(
                        value =
                            "${lunarDate.day}${if(lunarDate.day == 1) "/${lunarDate.month}" else ""}",
                        color = Color.Unspecified,
                    ),
                fullDisplayDay =
                    DisplayField(
                ...
                observance =
                    lunarDate.observance?.let { DisplayField(value = it, color = HolidayBlue) },
                lucDieu =
                    DisplayField(
